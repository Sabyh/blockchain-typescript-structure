...
const trpcRouter = t.router({
    ...
    sendTransaction: t.procedure
        ...
        .mutation(async ({ input }) => {
        const blockchainService = BlockchainFactory.getInstance(input?.type || 'bitcoin', input?.networkType); // Default to 'bitcoin' if type is not provided
        return await blockchainService.sendTransaction(input?.fromAddress || '', input?.privateKey || '', input?.toAddress || '', input?.amount || 0);
    }),
    ...
    createAgreement: t.procedure
        ...
        await agreementInstance.storeAgreement(input?.agreementType, input?.agreementNo, input?.clientName, input?.partyName, input?.agreementDate, input?.expiryDate, input?.ipfsHash);
        return { success: true };
    }),
    updateAgreement: t.procedure
        ...
        .mutation(async ({ input }) => {
        const agreementInstance = new AgreementFactory();
        await agreementInstance.updateAgreement(input?.agreementNo, input?.clientName, input?.partyName, input?.expiryDate, input?.ipfsHash);
        return { success: true };
    }),
    ...
    uploadPDF: t.procedure
        ...
        path: z.string().optional(), // Optionally accept a path for the file
    }))
        .mutation(async ({ input }) => {
        // Call the function to upload the PDF buffer
        const cid = await uploadPDFWithHelia(input.pdfBuffer, input.path || 'example.pdf');
        return { cid };
    }),
});
module.exports = {
